...
**0.1.0-alpha.4 源码预览**用具体描述说明 **Agent 正在做什么，接下来可以怎样推进**，
内容来自最近的请求、Agent 回应和已保存计划。例如“提交审核，通过后重新部署”，点击 **推进此建议**
就会把这条指令交给原 Agent；忙碌时先排队，空闲后发送。点击 **查看会话** 可查看原始上下文。
选中的 Topic、Project，以及当前会话所属的父级分组都会保持高亮。
Topic 仍可编辑目标、最多三条下一步和卡点，保存后跨重启保留；人和 Agent 可通过界面、CLI 和 socket API
查看概览、发起跟进。[使用说明](docs/project-overview.md)。
