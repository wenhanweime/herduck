...
fn server_reload_agent_manifests_reports_runtime_override() {
    ...
    let override_dir = config_home.join("herduck-dev").join("agent-detection");
...
fn pane_info_and_subscriptions_expose_done_agent_status() {
    ...
    let working = send_request(
        &socket_path,
        &format!(
            r#"{{"id":"req_status_3","method":"pane.report_agent","params":{{"pane_id":"{}","source":"herdr:pi","agent":"pi","state":"working"}}}}"#,
            background_pane_id
        ),
    );
    assert_eq!(working["result"]["type"], "ok");
    let idle = send_request(
        &socket_path,
        &format!(
            r#"{{"id":"req_status_4","method":"pane.report_agent","params":{{"pane_id":"{}","source":"herdr:pi","agent":"pi","state":"idle"}}}}"#,
            background_pane_id
        ),
    );
    assert_eq!(idle["result"]["type"], "ok");

    let status_event = reader
        .try_read_json_line(Duration::from_secs(2))
        .unwrap_or_else(|| {
            let pane = send_request(
                &socket_path,
                &format!(
                    r#"{{"id":"req_status_timeout","method":"pane.get","params":{{"pane_id":"{}"}}}}"#,
                    background_pane_id
                ),
            );
            panic!("timed out waiting for done status; pane snapshot: {pane}");
        });
