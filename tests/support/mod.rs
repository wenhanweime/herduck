...
pub fn isolate_project_history_for_pty(
    ...
    let home = runtime_dir.join("project-history-home");
    let data = runtime_dir.join("project-history-data");
    std::fs::create_dir_all(&home).expect("create isolated project history home");
    std::fs::create_dir_all(&data).expect("create isolated project history data");
