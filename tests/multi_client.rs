...
fn server_log_path(config_home: &Path) -> PathBuf {
    ...
    config_home.join(app_dir).join("herduck-server.log")
}
...
fn multi_client_broadcasts_frame_updates_to_all_clients() {
    ...
    send_client_input(&mut client_a, format!("echo {marker}\r").as_bytes());
...
fn multi_client_client_crash_sigkill_does_not_affect_server() {
    ...
    send_client_input(&mut survivor, b"echo survivor-still-works\r");
    assert!(
