...
fn project_api_contract_round_trips_with_dot_names() {
    let snapshot_request = Request {
        ...
        method: Method::ProjectSnapshot(ProjectSnapshotParams {
            projects_schema_version: crate::projects::domain::PROJECTS_SCHEMA_VERSION,
        }),
    };
    let snapshot_json = serde_json::to_value(&snapshot_request).unwrap();
    assert_eq!(snapshot_json["method"], "project.snapshot");
    assert_eq!(
        serde_json::from_value::<Request>(snapshot_json).unwrap(),
        snapshot_request
    );

    let assign_request = Request {
        id: "project_assign".into(),
        method: Method::ProjectSessionAssign(ProjectSessionAssignParams {
            session_key: "session-1".into(),
            project_key: "project-1".into(),
            locked: true,
        }),
    };
    let assign_json = serde_json::to_value(&assign_request).unwrap();
    assert_eq!(assign_json["method"], "project.session.assign");
    assert_eq!(
        serde_json::from_value::<Request>(assign_json).unwrap(),
        assign_request
    );

    let unlock_request = Request {
        id: "project_unlock".into(),
        method: Method::ProjectSessionUnlock(ProjectSessionUnlockParams {
            session_key: "session-1".into(),
        }),
    };
    let unlock_json = serde_json::to_value(&unlock_request).unwrap();
    assert_eq!(unlock_json["method"], "project.session.unlock");
    assert_eq!(
        serde_json::from_value::<Request>(unlock_json).unwrap(),
        unlock_request
    );

    let page_request = Request {
        id: "project_page".into(),
        method: Method::ProjectSessionsPage(ProjectSessionsPageParams {
            projects_schema_version: crate::projects::domain::PROJECTS_SCHEMA_VERSION,
            project_key: "project-1".into(),
            cursor: Some(crate::projects::SessionCursor {
                last_activity_at: 10,
                stable_key: "session-1".into(),
            }),
            limit: Some(50),
        }),
    };
    let page_json = serde_json::to_value(&page_request).unwrap();
    assert_eq!(page_json["method"], "project.sessions.page");
    assert_eq!(
        serde_json::from_value::<Request>(page_json).unwrap(),
        page_request
    );
}

#[test]
fn project_snapshot_response_and_update_event_round_trip() {
    let response = SuccessResponse {
        id: "project_snapshot".into(),
        result: ResponseResult::ProjectSnapshot {
            snapshot: Box::new(crate::projects::domain::ProjectsSnapshot::empty()),
        },
    };
    let response_json = serde_json::to_value(&response).unwrap();
    assert_eq!(response_json["result"]["type"], "project_snapshot");
    assert_eq!(
    ...
    let event = EventEnvelope {
        event: EventKind::ProjectSnapshotUpdated,
        data: EventData::ProjectSnapshotUpdated {
            revision: 7,
            scan_status: vec![AdapterScanStatus {
                adapter: "codex".into(),
                state: "complete".into(),
    ...
    assert_eq!(event.event.dot_name(), "project.snapshot.updated");
    assert_eq!(event_json["event"], "project_snapshot_updated");
    assert_eq!(
        serde_json::from_value::<EventEnvelope>(event_json).unwrap(),
        event
    );
}
