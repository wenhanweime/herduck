...
pub fn configure_from_args(args: &[String]) -> Result<Vec<String>, String> {
        ...
        && args.get(2).map(String::as_str) == Some("attach")
    {
        ...
        let Some(name) = args.get(3) else {
            return Err("usage: herduck session attach <name>".to_string());
        };
        if args.len() != 4 {
            return Err("usage: herduck session attach <name>".to_string());
        }
    ...
    } else if socket_override_from_env().is_some() {
...
pub fn local_attach_command() -> String {
    let product = crate::build_info::PRODUCT_NAME;
    match active_name() {
        Some(name) => format!("{product} session attach {name}"),
...
pub fn stop_command_for(name: Option<&str>) -> String {
    let product = crate::build_info::PRODUCT_NAME;
    match name {
        Some(name) => format!("{product} session stop {name}"),
        None => format!("{product} server stop"),
    }
}
...
pub fn restart_after_update_guidance(stop_command: &str, attach_command: Option<&str>) -> String {
    ...
    let restart = match attach_command {
        ...
        None => {
            format!("Run `{stop_command}`, then restart {product} with the same socket override.")
        }
    };
    format!(
...
pub fn active_restart_after_update_guidance() -> String {
    if !explicit_session_requested() {
        if let Some(socket_path) = socket_override_from_env() {
...
pub fn active_api_socket_path() -> PathBuf {
    ...
    if let Some(path) = socket_override_from_env() {
...
mod tests {
    ...
    fn local_attach_command_uses_default_launch_for_default_session() {
        ...
        assert_eq!(local_attach_command(), "herduck");
    }
    ...
    fn local_attach_command_uses_session_attach_for_named_session() {
        ...
        assert_eq!(local_attach_command(), "herduck session attach work");
    ...
    fn local_stop_command_uses_server_stop_for_default_session() {
        ...
        assert_eq!(local_stop_command(), "herduck server stop");
    ...
    fn local_stop_command_uses_session_stop_for_named_session() {
        ...
        assert_eq!(local_stop_command(), "herduck session stop work");
    ...
    fn restart_after_update_guidance_names_stop_and_attach_commands() {
        assert_eq!(
            restart_after_update_guidance(
                "herduck session stop work",
                Some("herduck session attach work")
            ),
            "Stop the old server to use the new version.\nStopping exits pane processes.\nRun `herduck session stop work`, then run `herduck session attach work` again."
        );
    }
    ...
    fn active_restart_after_update_guidance_respects_socket_override() {
        ...
        assert_eq!(
            ...
            "Stop the old server to use the new version.\nStopping exits pane processes.\nRun `HERDUCK_SOCKET_PATH=/tmp/herduck-custom.sock herduck server stop`, then restart herduck with the same socket override."
        );
    ...
    fn env_socket_override_wins_without_explicit_session() {
        ...
        std::env::set_var(SESSION_ENV_VAR, "work");
        clear_explicit_session_for_test();
        // The test runner may itself be running inside a Herdr pane, which would otherwise make
        // this deliberate override look inherited.
        std::env::remove_var(crate::HERDR_ENV_VAR);
    ...
    fn env_socket_override_skips_invalid_env_session_validation_without_explicit_session() {
        ...
        std::env::set_var(SESSION_ENV_VAR, "bad/name");
        clear_explicit_session_for_test();
        std::env::set_var(crate::api::SOCKET_PATH_ENV_VAR, "/tmp/herduck.sock");
        let args = vec![
            "herdr".to_string(),
            "workspace".to_string(),
            "list".to_string(),
        ...
        assert_eq!(cleaned, vec!["herdr", "workspace", "list"]);
        assert!(!explicit_session_requested());
        assert_eq!(active_api_socket_path(), PathBuf::from("/tmp/herduck.sock"));
