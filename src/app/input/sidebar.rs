...
mod tests {
    ...
    #[test]
    fn clicking_agent_panel_header_preserves_order() {
        let mut app = app_for_mouse_test();
        ...
        let header = Rect::new(detail_area.right() - 7, detail_area.y + 1, 7, 1);
        app.handle_mouse(mouse(
            MouseEventKind::Down(MouseButton::Left),
            header.x,
            header.y,
    ...
    #[test]
    fn clicking_collapsed_agent_row_ignores_legacy_priority_setting() {
        let mut app = app_for_mouse_test();
        let first = Workspace::test_new("one");
        let first_pane = first.tabs[0].root_pane;
        let second = Workspace::test_new("two");
