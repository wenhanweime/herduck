...
use crate::{
    app::state::{
        ...
        MenuListState, Mode, ProjectFilter, ProjectGrouping, ProjectTreeAction,
        RightClickPassthroughGesture, SidebarView, TabPressState, ViewLayout, WorkspacePressState,
    },
    ...
    pub(super) fn handle_mouse(
        ...
        match mouse.kind {
            MouseEventKind::Down(MouseButton::Left) => {
                ...
                if in_sidebar && !self.sidebar_collapsed && self.sidebar_view.is_project_browser() {
                    ...
                    if rect_contains(self.view.project_search_rect, mouse.column, mouse.row) {
                        self.projects.search_focused = true;
                        self.mode = Mode::Navigate;
                        return None;
                    }
            ...
            MouseEventKind::ScrollUp if in_sidebar => {
                if self.sidebar_view.is_project_browser() && !self.sidebar_collapsed {
                    self.projects.scroll = self.projects.scroll.saturating_sub(1);
                    return None;
                }
