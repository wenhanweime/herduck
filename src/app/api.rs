...
impl App {
    ...
    pub(crate) fn handle_internal_event(&mut self, ev: AppEvent) {
        ...
        if let Some((pane_id, force_metadata_refresh)) = project_runtime_sync {
            self.sync_project_runtime_for_pane(pane_id, force_metadata_refresh);
        }
    ...
    pub(crate) fn handle_api_request_after_internal_events_drained(
        ...
        let response = match request.method {
            ...
            Method::ProjectSnapshot(params) => {
                    ...
                    != crate::projects::domain::PROJECTS_SCHEMA_VERSION
                {
                    return responses::encode_error(
                        request.id,
                        "unsupported_version",
                        "unsupported Projects schema version",
                    );
                }
                return responses::encode_success(
                    request.id,
                    ResponseResult::ProjectSnapshot {
                        snapshot: Box::new(self.project_service.snapshot()),
                    },
                );
            }
            Method::ProjectSessionsPage(params) => {
                if params.projects_schema_version
                    != crate::projects::domain::PROJECTS_SCHEMA_VERSION
                {
                    return responses::encode_error(
                ...
                return match self.project_service.sessions_page(
                    params.project_key,
                    params.cursor,
                    usize::from(params.limit.unwrap_or(50)),
                ) {
                    Ok(page) => responses::encode_success(
                        request.id,
                        ResponseResult::ProjectSessionsPage {
                            page: Box::new(page),
                        },
                    ),
            ...
            Method::ProjectSessionAssign(params) => {
                let observed_at = std::time::SystemTime::now()
                    .duration_since(std::time::UNIX_EPOCH)
                    .unwrap_or_default()
                    .as_millis()
                    .min(i64::MAX as u128) as i64;
                return match self.project_service.assign_session(
                    params.session_key,
                    params.project_key,
                    params.locked,
                    observed_at,
                    ...
                    Ok(revision) => responses::encode_success(
                        request.id,
                        ResponseResult::ProjectSessionAssigned {
                            revision,
            ...
            Method::ProjectSessionUnlock(params) => {
                ...
                return match self
                    .project_service
                    .unlock_session(params.session_key, observed_at)
                {
                    Ok(revision) => responses::encode_success(
                        request.id,
                        ResponseResult::ProjectSessionAssigned {
...
mod tests {
    ...
    #[test]
    fn project_api_reads_and_mutates_the_server_owned_catalog() {
        let event_hub = crate::api::EventHub::default();
        let (_api_tx, api_rx) = tokio::sync::mpsc::unbounded_channel();
        let mut app = App::new(
            &crate::config::Config::default(),
        ...
        app.project_service = crate::projects::ProjectService::in_memory(event_hub.clone());
        let identity = crate::projects::SessionIdentity::id("codex", "api-session")
        ...
        let snapshot_response = app.handle_api_request(crate::api::schema::Request {
            id: "project_snapshot".into(),
            method: crate::api::schema::Method::ProjectSnapshot(
                crate::api::schema::ProjectSnapshotParams {
                    projects_schema_version: crate::projects::domain::PROJECTS_SCHEMA_VERSION,
                },
            ),
        });
        let snapshot_response: serde_json::Value =
            serde_json::from_str(&snapshot_response).expect("snapshot response JSON");
        assert_eq!(snapshot_response["result"]["type"], "project_snapshot");
        let project_key = snapshot_response["result"]["snapshot"]["projects"][0]["canonical_key"]
            .as_str()
            .expect("project key")
            .to_string();
        assert_eq!(
            snapshot_response["result"]["snapshot"]["projects"][0]["sessions"][0]["stable_key"],
            session_key
        );

        let page_response = app.handle_api_request(crate::api::schema::Request {
            id: "project_page".into(),
            method: crate::api::schema::Method::ProjectSessionsPage(
                crate::api::schema::ProjectSessionsPageParams {
                    projects_schema_version: crate::projects::domain::PROJECTS_SCHEMA_VERSION,
        ...
        let page_response: serde_json::Value =
            serde_json::from_str(&page_response).expect("page response JSON");
        assert_eq!(page_response["result"]["type"], "project_sessions_page");
        assert_eq!(
        ...
        let assign_response = app.handle_api_request(crate::api::schema::Request {
            id: "project_assign".into(),
            method: crate::api::schema::Method::ProjectSessionAssign(
                crate::api::schema::ProjectSessionAssignParams {
        ...
        let assign_response: serde_json::Value =
            serde_json::from_str(&assign_response).expect("assign response JSON");
        assert_eq!(
            assign_response["result"]["type"],
            "project_session_assigned"
        );
        assert_eq!(assign_response["result"]["locked"], true);
        assert!(event_hub.current_sequence() >= 2);

        let unlock_response = app.handle_api_request(crate::api::schema::Request {
            id: "project_unlock".into(),
            method: crate::api::schema::Method::ProjectSessionUnlock(
                crate::api::schema::ProjectSessionUnlockParams { session_key },
            ),
        });
        let unlock_response: serde_json::Value =
            serde_json::from_str(&unlock_response).expect("unlock response JSON");
        assert_eq!(unlock_response["result"]["locked"], false);
    }

    #[test]
    fn project_snapshot_rejects_an_unsupported_schema_version() {
        let (_api_tx, api_rx) = tokio::sync::mpsc::unbounded_channel();
        let mut app = App::new(
            &crate::config::Config::default(),
            true,
        ...
        let response = app.handle_api_request(crate::api::schema::Request {
            id: "unsupported_projects_schema".into(),
            method: crate::api::schema::Method::ProjectSnapshot(
                crate::api::schema::ProjectSnapshotParams {
                    projects_schema_version: crate::projects::domain::PROJECTS_SCHEMA_VERSION + 1,
                },
            ),
        });
        let response: serde_json::Value =
            serde_json::from_str(&response).expect("error response JSON");
