    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ClientError::ConnectionFailed(err) => {
                ...
                write!(
                    f,
                    "\nIs {product} server running? Start it with `{product} server`.",
    ...
    let _input_thread = std::thread::spawn(move || {
        ...
        for line in stdin.lock().lines() {
            ...
            match terminal_control_command_from_json(&line) {
                ...
                Err(err) => eprintln!(
                    "{}: terminal session control input ignored: {err}",
...
fn connect_terminal_session_stream(
    ...
    match do_handshake(
        ...
        Err(err) => {
            eprintln!("{}: {}", crate::build_info::PRODUCT_NAME, err);
            std::process::exit(1);
        }
    }
...
fn run_client_with_mode(
    ...
    let mut stream = match crate::ipc::connect_local_stream(&socket_path) {
        ...
        Err(err) => {
            ...
            eprintln!("{}: {}", crate::build_info::PRODUCT_NAME, client_err);
            std::process::exit(1);
        }
    };
    ...
    if let Some((terminal_id, takeover)) = attach_request {
        ...
        if let Err(err) = write_to_server(&mut stream, &attach) {
            eprintln!(
                "{}: failed to request terminal attach: {err}",
                crate::build_info::PRODUCT_NAME
            );
            std::process::exit(1);
        }
