...
pub(crate) use self::brand::{
    brand_menu_rect, sidebar_brand_footer_rect, sidebar_content_rect, sidebar_logo_rect,
    workspace_footer_rect, MIN_FOOTER_ACTIONS_WIDTH,
};
...
fn compute_view_internal(
    ...
    if !app.sidebar_collapsed && app.sidebar_view == crate::app::state::SidebarView::SpacesAgents {
    ...
    let workspace_card_areas = if app.sidebar_collapsed || app.sidebar_view.is_project_browser() {
        Vec::new()
    } else {
    ...
    if app.sidebar_view.is_project_browser() {
        ...
        app.projects.selected_row = app.projects.selected_row.min(row_count.saturating_sub(1));
    }
    app.view = crate::app::ViewState {
        ...
        pane_infos,
        split_borders,
        project_sidebar_tabs: project_geometry.sidebar_tabs,
...
pub fn render_with_runtime_registry(
    ...
    if app.view.layout == ViewLayout::Mobile {
        ...
        } else if app.sidebar_view.is_project_browser() {
            render_projects_sidebar(app, frame, sidebar_area);
        } else {
